...
var (
	...
	// toolkitSlogLogger provides a slog logger for exporter-toolkit. Passing nil causes a panic in
	// current exporter-toolkit versions because slog.(*Logger).Info is called unconditionally.
	toolkitSlogLogger = slog.New(slog.NewTextHandler(os.Stdout, &slog.HandlerOptions{Level: slog.LevelInfo}))
)
...
func main() {
	...
	go func() {
		// Pass a non-nil slog.Logger to avoid nil pointer dereference inside exporter-toolkit.
		if errServe := web.ListenAndServe(server, toolkitFlags, toolkitSlogLogger); errServe != nil {
			errCheck(errServe)
		}
	}()
